...
La Subsección~4.1 definió los coeficientes locales \(\lambda^m\), \(\varpi^m\) y \(\gamma^m\), que representan, respectivamente, el retorno marginal asociado con el desempeño percibido \(P^m\), la valoración marginal del ahorro reconocido y el costo marginal total de una mayor multa. Para ordenar localmente los efectos reputacionales asociados con la señal publicada se supone

\begin{equation}
\label{eq:orden_legibilidad_icg}
\ell_I^B
\geq
\ell_I^{A''}
=
...
El ordenamiento refleja la claridad con que la señal publicada comunica el desempeño. En \(A_0\), \(P^{A_0}\) combina resultados del servicio, otras metas de gestión, el avance financiero general y la relación de trabajo. En \(A\), la salida de la relación de trabajo y la focalización del componente financiero en \(R^{F}\) reducen esta heterogeneidad. Entre \(A\), \(A'\) y \(A''\) la legibilidad se mantiene constante como normalización local para aislar el efecto de la composición reputacional, mientras que en \(B\) la concentración de \(P^B\) en \(C_S\) permite asociar más directamente sus variaciones con los resultados del servicio.
...
Por construcción, \(f_x^m=0\) cuando \(r\notin\mathcal U_F^m\). Cuando \(r\in\mathcal U_F^m\), se supone
...
Mientras \(d_F^m\) permanezca constante y \(r\in\mathcal U_F^m\), se tiene \(K_{r,e}^{m}>0\) y \(K_{r,g}^{m}<0\). Utilizando además la multiplicidad \(\delta_r^m\) definida en la Subsección~4.2, los efectos continuos de los esfuerzos sobre la multa pueden escribirse como
...
Dado que \(\lambda^m=V'(\mathcal R^m)d_I\ell_I^mR_I'(P^m)\), la mayor legibilidad de \(B\) facilita el cumplimiento de \eqref{eq:condicion_lambda_b}, pero no lo garantiza: cambios suficientemente grandes en \(V'(\mathcal R^m)\) o \(R_I'(P^m)\) pueden compensar ese efecto.
...
\right]
C_{S,e_r}
}_{\text{ganancia por focalización y legibilidad}}
>0,
\\[3pt]
L_P
&\equiv
...
\alpha_{RT}^{A_0}c_{RT,e_r}
\right]
}_{\text{pérdida de canales reputacionales}}
\geq0,
\\[3pt]
L_M
&\equiv
...
K_{r,e}^{A_0}
\right]
}_{\text{pérdida de canales sancionadores}}
>0.
\end{aligned}
\end{equation}

La desigualdad \(G_P>0\) se deriva conjuntamente de \(\ell_I^{A}>\ell_I^{A_0}\), \eqref{eq:mayor_peso_cs_a_a0} y \(C_{S,e_r}>0\). Como la diferencia total de los canales continuos es exactamente \(G_P-L_P-L_M\), en esta región \(A\) genera un mayor incentivo local de ejecución si y solo si
...
\subsubsection{Bienestar regulatorio}

El último paso traslada los resultados agregados al criterio del regulador definido en \eqref{eq:bienestar_regulador}. Un mayor esfuerzo o un mejor resultado del servicio no bastan para preferir un esquema: \(Q^m=Q(\widetilde q_S^m,C_G^m)\) valora también las metas de gestión que contribuyen a esos resultados, y la comparación incorpora los costos de operación y mantenimiento y la carga pecuniaria impuesta a la empresa. El regulador evalúa estos componentes tomando como dadas las respuestas óptimas \((\mathbf e^{m,*},\mathbf g^{m,*})\).
...
-
M^{m'},
\label{eq:cambio_multa_bienestar}
\\
\Delta_{OM}^{m,m'}
&\equiv
C_{OM}^{m'}
...
y la condición para una mejora de bienestar es
...
La propiedad de saturación de la Subsección~4.1 permite precisar el costo regulatorio de una exposición sancionadora adicional. Supóngase que, una vez alcanzada la región \(e_r\geq e_r^{\mathrm{sat}}\), una variación marginal de \(\mu_r^m\) no modifica localmente \(\widetilde q_S^m\), \(C_G^m\) ni \(C_{OM}^m\). Si \(\bar a_r<1\), a partir de \eqref{eq:exposicion_sancionadora_r} se obtiene
...
Así, una vez agotado el margen de culminación, una mayor exposición deja de aportar mejoras por esa vía y, si tampoco mejora otros componentes del criterio regulatorio, reduce \(\mathcal W_R^m\). En particular, si dos diseños mantienen el avance físico saturado y generan localmente los mismos \(Q\) y \(C_{OM}\), el diseño con menor exposición sancionadora obtiene mayor bienestar regulatorio. Esta comparación constituye una mejora de eficiencia sancionadora y no una dominancia global cuando los diseños modifican simultáneamente otros canales.
...
Esta expresión identifica únicamente la contribución de la exposición de \(r\); el orden de la multa total requiere considerar los demás componentes e intervenciones. En términos agregados, una condición suficiente para \(\mathcal W_R^A>\mathcal W_R^{A_0}\) es
...
\begin{resultadobox}
{Bienestar regulatorio: \textit{benchmark} \(A_0\to A\)}
\label{res:bienestar_a0_a}
Si las exposiciones de \(A_0\) y \(A\) son suficientes para saturar el margen de culminación, \(\mu_r^{A}<\mu_r^{A_0}\), y \eqref{eq:condicion_focalizacion_a_a0} se cumple para las intervenciones relevantes de \(R^{S}\), entonces \(\widetilde q_S^A\geq\widetilde q_S^{A_0}\). Si, además,
\begin{equation}
Q^{A}
\geq
Q^{A_0},
...
\mathcal W_R^{A_0}.
\end{equation}
Fuera de estas condiciones, el orden permanece abierto.
\end{resultadobox}

\paragraph{Secuencia principal \(A\to A'\to A''\to B\).}

En cada transición se aplica \eqref{eq:condicion_general_mejora_bienestar}. En \(A\to A'\), el cambio en \(Q\) y la eliminación del umbral deben compensar cualquier aumento de la multa que resulte de activar consecuencias antes desactivadas. En \(A'\to A''\), el fortalecimiento de los retornos reputacionales de \(C_S\) y \(C_G\), junto con la corrección de los incentivos de eficiencia de \(R^{F}\), debe compensar cualquier cambio desfavorable derivado de sustituir la base financiera por una física.
...
Por tanto, la mejora de los resultados directos del servicio debe ser suficiente para compensar, cuando exista, la pérdida asociada con el menor cumplimiento de \(J^G\), además de las diferencias en multas y costos de operación y mantenimiento. Localmente, el canal de resultados favorece a \(B\) cuando
...
\begin{resultadobox}
{Mejora de bienestar en la secuencia principal}
\label{res:bienestar_secuencia_principal}
Para cada transición consecutiva \(m\to m'\) de \(A\to A'\to A''\to B\), si se cumple \eqref{eq:condicion_general_mejora_bienestar}, entonces
\begin{equation}
\mathcal W_R^{m'}
>
\mathcal W_R^{m}.
\end{equation}
En \(A''\to B\), esta condición debe incorporar explícitamente la posible oposición entre la mejora de \(\widetilde q_S\) y la reducción de \(C_G\). Las fuentes de la ganancia pueden diferir entre transiciones; por ello, el resultado es condicionado y no se deriva únicamente del ordenamiento de los resultados directos del servicio.
\end{resultadobox}

Combinando el Resultado~\ref{res:bienestar_a0_a} con el Resultado~\ref{res:bienestar_secuencia_principal}, si cada eslabón satisface su condición suficiente, por transitividad se obtiene

\begin{equation}
\label{eq:ordenamiento_bienestar_condicionado}
\mathcal W_R^{B}
>
\mathcal W_R^{A''}
>
...
\begin{resultadobox}
{Ordenamiento condicionado de bienestar regulatorio}
\label{res:ordenamiento_bienestar_condicionado}
Si \(A\) satisface las condiciones del Resultado~\ref{res:bienestar_a0_a} respecto de \(A_0\) y cada transición de la secuencia principal satisface \eqref{eq:condicion_general_mejora_bienestar}, entonces se cumple \eqref{eq:ordenamiento_bienestar_condicionado}. La cadena no constituye una dominancia automática de los esquemas posteriores: cada eslabón requiere una ganancia regulatoria neta positiva.
\end{resultadobox}
