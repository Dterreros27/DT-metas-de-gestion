...
El indicador \(\zeta_j\) toma el valor uno cuando el resultado adicional es verificable, se obtiene bajo las condiciones de recursos reconocidas para las intervenciones correspondientes y satisface los demás criterios de elegibilidad. Así, \(h_j\) es nulo cuando no existe sobrecumplimiento elegible, aumenta con \(q_j\) dentro del rango reconocido y queda acotado en \(\bar h_j\). Las comparaciones marginales posteriores se realizan dentro de una región en la que \(\zeta_j\) permanece constante; un cambio en su valor constituye un cambio de estado.
...
La segunda condición impide reconocer un rango de sobrecumplimiento superior al resultado máximo compatible con la tecnología del modelo.
...
\begin{figure}[!t]
    ...
    \includegraphics[width=\columnwidth]{Imagen/bonif_sobrecumpl.png}
...
El monto reconocido mediante \(\mathcal P\) se incorpora tarifariamente y permanece afectado al financiamiento de inversiones. Para evitar una doble contabilización, \(S^{B^{+}}\) recoge los recursos que permanecen efectivamente sin utilizar después de ejecutar el alcance que genera el resultado observado, mientras que \(\mathcal P\) representa el reconocimiento regulatorio asociado con el sobrecumplimiento. Los recursos utilizados para producir el resultado adicional no se contabilizan simultáneamente como ahorro. En el canal reputacional, ambas formas de valor reconocido se consideran conjuntamente mediante \(R_E(S^{B^{+}}+\mathcal P)\).
...
Se supone que \(\mathcal P\) no es apropiable por el gerente y no entra como compensación monetaria directa en \(U_G^{B^{+}}\). Su efecto opera únicamente a través de la valoración reputacional de los recursos reconocidos. Como \(B\) concentra la señal publicada en los resultados del servicio, el desempeño percibido bajo la extensión se define como
...
\\
\frac{\partial \mathcal P}{\partial e_r}
&=
\kappa_j b_j
\beta_{rj}
\frac{\partial F_r}{\partial e_r}
...
Para una intervención completada, y manteniendo localmente constantes el resultado observado y \(\mathcal P\),

\begin{equation}
\label{eq:incentivo_eficiencia_bplus}
\frac{\partial (S^{B^{+}}+\mathcal P)}{\partial g_r}
=
-
...
Q^{B^{+}}
=
Q\!\left(\widetilde q_S^{B^{+}},C_G^{B}\right)
\geq
Q\!\left(\widetilde q_S^{B},C_G^{B}\right)
=
Q^{B}.
...
El mayor desempeño potencial de \(B^{+}\) no implica por sí solo una mejora de bienestar. A los tres componentes definidos en la Subsección~4.3 para una transición \(m\to m'\) se añade ahora el costo regulatorio neto de financiar la bonificación. Sea \(\mathcal C_{\mathcal P}(\mathcal P)\) dicho costo, con

\begin{equation}
\label{eq:costo_bonificacion_bplus}
\mathcal C_{\mathcal P}(0)=0,
\qquad
\mathcal C_{\mathcal P}'(\mathcal P)>0.
\end{equation}

Si cada unidad reconocida mediante \(\mathcal P\) se contabiliza íntegramente como costo, el caso particular es \(\mathcal C_{\mathcal P}(\mathcal P)=\mathcal P\). Si parte del reconocimiento conserva valor regulatorio por permanecer afectado al financiamiento de inversiones, \(\mathcal C_{\mathcal P}(\mathcal P)\) representa únicamente el costo neto correspondiente.
...
M^{B^{+}}
-
\mathcal C_{\mathcal P}(\mathcal P),
\end{equation}
...
Utilizando las definiciones generales de \(\Delta_\Phi^{m,m'}\), \(\Delta_M^{m,m'}\) y \(\Delta_{OM}^{m,m'}\) de la Subsección~4.3, se añade

\begin{equation}
\label{eq:delta_p_bplus}
\Delta_{\mathcal P}^{B,B^{+}}
\equiv
\mathcal C_{\mathcal P}(\mathcal P).
\end{equation}
...
La igualdad \(M^{B^{+}}=M^B\) de \eqref{eq:multa_bplus} se refiere a un mismo nivel de cumplimiento. En equilibrio, el mayor esfuerzo inducido por \(B^{+}\) puede reducir las bases sancionadoras y, con ello, generar \(\Delta_M^{B,B^{+}}\geq0\). Este canal puede reforzar la ganancia de bienestar, pero no es necesario para que la extensión resulte preferible.
