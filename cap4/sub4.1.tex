...
El avance financiero requiere un tratamiento adicional, pues una ejecución financiera inferior a la programada no necesariamente implica que exista alcance físico pendiente. En los esquemas en los que se evalúa esta obligación, sea \(f^m\) la medida aplicable y \(\mathcal U_F^m\subseteq R\) el conjunto de inversiones comprendidas en dicha medida, de modo que
...
Una misma inversión puede incidir en más de una base cuando participa simultáneamente en una meta específica y en una obligación de alcance transversal. Esta superposición es relevante porque puede aumentar la exposición sancionadora atribuible a una misma brecha de ejecución. La composición de las bases aplicables, el universo financiero \(\mathcal U_F^m\) y la regla de activación \(\chi^m\in\{0,1\}\) dependen del diseño regulatorio considerado.
...
Los esfuerzos también pueden modificar las condiciones en las que cambian \(\mathbf 1\{c_j<1\}\), \(d_F^m\), \(\chi^m\) o el reconocimiento de \(S_r\). Sean \(\mathcal D_{r,e}^m\) y \(\mathcal D_{r,g}^m\) los efectos asociados con estos cambios discretos. Las condiciones marginales pueden expresarse entonces como
...
Se supone que \(\Gamma_r^m(\mu)\) es continua y creciente en \(\mu\) en la región relevante. Esta expresión incorpora tanto la probabilidad de detección \(p\) como la transmisión monetaria \(\rho\) y el efecto reputacional de la sanción a través de \(\gamma^m\).

Para separar este canal de los demás incentivos, sea \(\mathcal C_{r,e}^m(e_r)\) el beneficio marginal de ejecución proveniente del desempeño reputacional, el ahorro, otros componentes de la multa y los cambios discretos, excluyendo el efecto directo de reducir la brecha \(1-a_r\) en \eqref{eq:exposicion_sancionadora_r}. En el tramo no saturado, la condición marginal puede escribirse como

\begin{equation}
\label{eq:foc_probabilidad_cumplimiento}
\psi_r'(e_r)
=
\mathcal C_{r,e}^{m}(e_r)
+
a_r'(e_r)
\Gamma_r^m(\mu_r^m),
\qquad
...
-
\mathcal C_{r,e}^{m}(e_r^{\mathrm{sat}})
>
0.
\end{equation}
...
La exposición suficiente depende de los demás incentivos que actúan sobre la misma inversión. Cuanto mayor sea el retorno proveniente del desempeño reputacional, del ahorro u otros canales recogidos en \(\mathcal C_{r,e}^{m}\), menor será, manteniendo lo demás constante, la exposición sancionadora necesaria para llevar el esfuerzo de ejecución hasta su región saturada. Se define \(\mu_r^{m,*}=0\) cuando \(\mathcal C_{r,e}^{m}(e_r^{\mathrm{sat}})\geq\psi_r'(e_r^{\mathrm{sat}})\), pues los canales restantes son suficientes para alcanzar dicha región.
