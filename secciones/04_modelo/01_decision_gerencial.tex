...
Bajo las condiciones de concavidad local, para \(\mu_r^m<\mu_r^{m,*}\) una mayor exposición puede seguir incrementando el esfuerzo a través de \(a_r'(e_r)\Gamma_r^m(\mu_r^m)\). Cuando \(\mu_r^m\geq\mu_r^{m,*}\), la exposición es suficiente para llevar el esfuerzo al punto en que el avance físico se satura. Como \(e_r\) solo produce beneficios mediante variables que dependen de \(a_r\) y \(\psi_r'(e_r)>0\), ningún óptimo elige esfuerzo estrictamente mayor que \(e_r^{\mathrm{sat}}\), manteniendo \(g_r\) fijo. Por tanto,
...
Esta medida permite que mejoras de los resultados por encima de las metas regulatorias sigan siendo valoradas por el regulador, aun cuando ya no modifiquen \(C_S\). El costo de inversión inducido por cada esquema se define como
...
Sobre esta base, se define el criterio reducido de bienestar regulatorio como
...
\begin{equation}
\label{eq:propiedades_carga_multa}
\Omega(0)=0,
\qquad
\Omega'(M)>0.
\end{equation}
