...
Por construcción, \(d_F^m\in\{0,1\}\). Una ejecución financiera inferior a la programada configura incumplimiento únicamente cuando existe alcance físico pendiente entre las inversiones evaluadas. En cambio, una brecha financiera originada exclusivamente en el ahorro generado por inversiones culminadas no activa \(d_F^m\). Cuando el incumplimiento se determina a partir del avance financiero, la base correspondiente se define como
...
\sum_{K\in\{S,G,F,RT\}}
s_K\,
\mathbf 1\{K\in\mathrm{Sig}^m\},
\\
\alpha_H^m
&=
\frac{
s_H\,
\mathbf 1\{H\in\mathrm{Sig}^m\}
}{
D^m
}.
\end{aligned}
...
donde \(D^m\) es el factor de normalización y \(\mathrm{Sig}^m\) denota el conjunto de bloques que integra la señal pública del esquema \(m\). En particular, se supone \(s_S>s_G\), de modo que los resultados del servicio poseen mayor saliencia reputacional que las demás metas de gestión. La medida \(f^m\) corresponde a \(f_R\) o \(f_F\), según el diseño aplicable. El \(ICG^m\) se mantiene como un objeto regulatorio distinto, construido con las ponderaciones establecidas por el regulador y utilizado en las reglas del esquema cuando corresponda.

La normalización anterior corresponde a una reasignación completa de la saliencia cuando un bloque deja de publicarse. Para mostrar qué resultados dependen de ese supuesto, se considera como ejercicio de sensibilidad la familia
\[
\widetilde\alpha_H^m(\theta)
=
\frac{s_H\mathbf 1\{H\in\mathrm{Sig}^m\}}{(D^m)^\theta},
\qquad
\theta\in[0,1].
\]
El caso \(\theta=1\) reproduce el \textit{benchmark} normalizado de \eqref{eq:ponderaciones_reputacionales}; con \(\theta=0\), la salida de un bloque no eleva mecánicamente el coeficiente de los restantes. En consecuencia, los ordenamientos que utilizan un aumento de \(\alpha_H^m\) deben interpretarse como condicionados al grado de reasignación de saliencia, y no como una consecuencia automática de retirar componentes del índice.
