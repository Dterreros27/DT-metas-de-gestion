...
Las Subsecciones~4.3 y~4.4 muestran que las modificaciones regulatorias afectan de manera distinta la ejecución, los resultados del servicio, el cumplimiento de otras metas, la eficiencia y el bienestar. La secuencia principal \(A\to A'\to A''\to B\) aísla cambios específicos del diseño; \(A_0\to A\) se mantiene como \textit{benchmark} institucional y \(B\to B^{+}\) incorpora el reconocimiento acotado del sobrecumplimiento. Todos los ordenamientos están sujetos a las condiciones metodológicas y locales establecidas previamente.
...
&
Mejora condicionada; véase el Resultado~\ref{res:bienestar_a0_a}.
\\[5pt]
...
&
Sujeto a \eqref{eq:condicion_general_mejora_bienestar}; véase el Resultado~\ref{res:bienestar_secuencia_principal}.
\\[5pt]
...
&
Sujeto a \eqref{eq:condicion_general_mejora_bienestar}, incluida la oposición entre \(\widetilde q_S\) y \(C_G\).
\\[5pt]
...
&
Sujeto a \eqref{eq:condicion_bienestar_bplus}; véase el Resultado~\ref{res:bienestar_bplus}.
\\
...
\begin{resultadobox}
{Ordenamiento condicionado con sobrecumplimiento}
\label{res:ordenamiento_bienestar_bplus}
Si \(A\) satisface las condiciones del Resultado~\ref{res:bienestar_a0_a} respecto de \(A_0\), cada transición de \(A\to A'\to A''\to B\) satisface \eqref{eq:condicion_general_mejora_bienestar} y \(B\to B^{+}\) satisface \eqref{eq:condicion_bienestar_bplus}, entonces

\begin{equation}
\label{eq:ordenamiento_bienestar_bplus}
W_R^{B^{+}}
>
W_R^{B}
>
...
La cadena constituye un ordenamiento condicionado y no una dominancia automática: cada transición requiere que el valor regulatorio de la modificación exceda sus costos.
\end{resultadobox}

En conjunto, el modelo separa progresivamente tres funciones que en los diseños iniciales aparecen parcialmente superpuestas: la evaluación regulatoria mediante el \(ICG^m\), la señal reputacional resumida por \(P^m\) y la estructura de consecuencias. La salida de componentes del ICG reasigna mecánicamente sus ponderaciones regulatorias, mientras que la composición de \(P^m\) determina, a partir de las saliencias \(s_H\), el peso reputacional de los bloques que permanecen. Esta separación puede corregir distorsiones, pero no implica que todas las transiciones mejoren simultáneamente todos los márgenes. Un diseño puede fortalecer los incentivos de una tarea específica o elevar los resultados directos del servicio y, aun así, no mejorar el bienestar regulatorio. Esta última conclusión exige que el efecto conjunto sobre \(Q(\widetilde q_S,C_G)\), la carga sancionadora y los costos sea favorable. Bajo las condiciones del modelo, \(B^{+}\) completa el ordenamiento condicionado como el esquema de mayor bienestar entre los considerados; fuera de ellas, la comparación debe realizarse transición por transición.
