...
El mayor desempeño potencial de \(B^{+}\) no implica por sí solo una mejora de bienestar. A los tres componentes definidos en la Subsección~4.3 para una transición \(m\to m'\) se añade ahora el costo regulatorio neto de financiar la bonificación. Sea \(C_T(T)\) dicho costo, con
...
Si cada unidad reconocida mediante \(T\) se contabiliza íntegramente como costo, el caso particular es \(C_T(T)=T\). Si parte del reconocimiento conserva valor regulatorio por permanecer afectado al financiamiento de inversiones, \(C_T(T)\) representa únicamente el costo neto correspondiente.
