...
Dado que \(\lambda^m=V'(\mathrm{Rep}^m)d_I\ell_I^mR_I'(P^m)\), la mayor legibilidad de \(B\) facilita el cumplimiento de \eqref{eq:condicion_lambda_b}, pero no lo garantiza: cambios suficientemente grandes en \(V'(\mathrm{Rep}^m)\) o \(R_I'(P^m)\) pueden compensar ese efecto.
...
Así, una vez agotado el margen de culminación, una mayor exposición deja de aportar mejoras por esa vía y, si tampoco mejora otros componentes del criterio regulatorio, reduce \(W_R^m\). En particular, si dos diseños mantienen el avance físico saturado y generan localmente los mismos \(Q\) y \(C_{OM}\), el diseño con menor exposición sancionadora obtiene mayor bienestar regulatorio. Esta comparación constituye una mejora de eficiencia sancionadora y no una dominancia global cuando los diseños modifican simultáneamente otros canales.
